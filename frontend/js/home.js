...
function setFeaturedCopy(personalized) {
  ...
  if (personalized) {
    title.textContent = 'Top Picks For You';
  ...
  <a href="recipe.html?id=${r.id}" class="recipe-card group block rounded-2xl overflow-hidden bg-white border border-slate-200/70 hover:-translate-y-1 transition-all duration-300">
    ...
    <div class="p-5">
      ...
      <div class="flex gap-2 text-xs font-medium text-slate-500">
        <span class="recipe-meta-chip px-2 py-1 rounded-md">${r.cook_time} min</span>
        <span class="recipe-meta-chip px-2 py-1 rounded-md">${r.difficulty || 'Med'}</span>
      </div>
    </div>
  </a>`;
}
...
function featuredCard(r, opts = {}) {
    ...
    return `
    <div class="relative rounded-3xl overflow-hidden shadow-2xl group h-[360px] md:h-[420px] border border-white/70">
        ${imageSources.length 
        ...
        <div class="absolute bottom-0 left-0 p-6 sm:p-8 md:p-12 w-full md:w-2/3 text-white">
            ...
            <h3 class="text-3xl sm:text-4xl md:text-5xl font-display font-extrabold mb-4 leading-tight group-hover:text-brand-300 transition-colors">${r.title}</h3>
...
async function loadFeatured() {
    ...
    if (token) {
        try {
            const response = await fetchWithAuth('/api/recommendations/top-pick');
            const topPicks = Array.isArray(response)
                ? response
            ...
            if (topPicks.length) {
                ...
                container.innerHTML = topPicksGrid(topPicks.slice(0, 3));
